...
command! -nargs=0 -bar SrcExplToggleDown
    \ call <SID>SrcExpl_Toggle_Down()

" Modify by Allen Lee
command! -nargs=0 -bar SrcExplRefresh
    \ call <SID>SrcExpl_Refresh_1()
"""""
...
" User interface for changing the Width of the Source Explorer window, Add by Allen Lee
if !exists('g:SrcExpl_winWidth')
...
" 0 is split, 1 is vsplit, Add by Allen Lee
let s:SrcExpl_isRunning_Down = 0
let s:SrcExpl_VorH = 0  
"""""
...
" Add by Allen Lee
function! <SID>SrcExpl_Refresh_1()
...
function! <SID>SrcExpl_WinNew(wincmd)
    ...
    " Modify by Allen Lee
    if s:SrcExpl_VorH == 0
        exe 'silent! botright ' . string(g:SrcExpl_winWidth) . 'vsplit ' . a:wincmd
    else
...
function! <SID>SrcExpl_Init()
    ...
    " Modify mark by Allen Lee, that cause ctag error: E429
...
function! <SID>SrcExpl_Init_Down()
...
function! <SID>SrcExpl_Toggle_Down()

    " Not yet running
    if s:SrcExpl_isRunning_Down == 0
        let s:SrcExpl_VorH = 1
        " Initialize the properties
        if <SID>SrcExpl_Init_Down()
            return -1
        endif
        " Create the window
        call <SID>SrcExpl_WinOpen()
        " We change the flag to true
        let s:SrcExpl_isRunning_Down = 1
    else
        " Not in the exact tab page
        " Mark by Allen Lee
        "if s:SrcExpl_tabPage != tabpagenr()
